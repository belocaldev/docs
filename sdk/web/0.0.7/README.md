...
## Rescanning and translating newly added elements

If your page adds new DOM nodes after the initial translation (for example after rendering a modal, loading a product list, or injecting CMS content), call:

```javascript
await window.BelocalWeb.rescan();
```

Behavior:

- `rescan()` runs the same scan + translate flow that is used during `init()`
- it translates only elements that were not yet translated into the currently chosen language
- existing already translated elements are not re-requested during this flow

Example:

```javascript
async function renderMoreProducts() {
  const container = document.querySelector('#products');
  container.insertAdjacentHTML('beforeend', `
    <article>
      <h3 data-i18n-translate>New arrival</h3>
      <p data-i18n-translate data-belocal-user-type="product">
...
| `data-belocal-user-type` | User type (e.g. `product`, `chat`) for translation context |
| `data-belocal-translation-style` | Overrides the project-level translation style: `auto`, `formal`, `informal`, `technical`, `marketing`, `literary` |
...
<p data-i18n-translate data-belocal-translation-style="formal">Welcome to our store</p>
```
